...
final class BookFlightViewController: UIViewController {
    ...
    private lazy var customHeaderView: CustomHeaderView = {
    ...
    private var segments: [SegmentDetails] = []
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        self.customHeaderView.resetText()
    }
    
    init(flightDetails: FlightDetails) {
        super.init(nibName: nil, bundle: nil)
        self.flightDetails = flightDetails
        for itinerary in flightDetails.itineraries {
            self.segments.append(contentsOf: itinerary.segments)
        }
    }
    ...
    private func setUpConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            customHeaderView.heightAnchor.constraint(greaterThanOrEqualToConstant: 45),
            ...
            self.view.safeAreaLayoutGuide.trailingAnchor.constraint(equalTo: customHeaderView.trailingAnchor),
            
            summaryTableView.topAnchor.constraint(equalTo: customHeaderView.bottomAnchor),
    ...
    private func getDelayPrediction() {
        for (index, segment) in segments.enumerated() {
            Constants.predictDelay(
                for: segment) { delayPrediction in
                    guard let delayPrediction = delayPrediction else { return }
                    self.customHeaderView.probabilityAndResult = ((index+1), delayPrediction.probability, delayPrediction.result)
                } onError: { error in
                    self.onFlightOrderError(error)
                }
        }
    }
